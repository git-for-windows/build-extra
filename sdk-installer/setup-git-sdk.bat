...
@IF NOT DEFINED JENKINS_URL @(
	...
	@bash --login -c "mkdir -p /usr/src && cd /usr/src && for project in MINGW-packages MSYS2-packages build-extra; do test ! -d $project && mkdir -p $project && (cd $project && git init && git config core.autocrlf false && git remote add origin https://github.com/git-for-windows/$project); done; if test ! -d git; then git clone -b @@GIT_BRANCH@@ --depth 1 -c core.autocrlf=false https://github.com/git-for-windows/git; fi && cd git && make install"
