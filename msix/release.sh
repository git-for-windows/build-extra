...
UCRT64)  ARTIFACT_SUFFIX="x64";;
CLANGARM64) ARTIFACT_SUFFIX=arm64;;
*) die "MSIX packages are not built for $MSYSTEM";;
esac

# MSIX ProcessorArchitecture matches the artifact suffix (x64/arm64).
# The arm64 package bundles x64 binaries that require POSIX emulation (e.g.
# bash.exe), so it needs the Windows 11 floor that supports x64-on-arm64
# emulation; the x64 package works on the older Windows 10 baseline.
case "$ARTIFACT_SUFFIX" in
arm64) MIN_VERSION="10.0.22000.0";;
*) MIN_VERSION="10.0.19041.0";;
esac

# Ship each release channel as its own MSIX package family so that stable, RC,
# and snapshot builds install side-by-side as independent apps. The channel is
# derived from the version string (snapshots use a "prerelease-" prefix, release
# candidates carry an "rc" marker); test the snapshot prefix first so a snapshot
# that also contains "rc" still maps to the snapshot family.
case "$VERSION" in
prerelease-*)
	IDENTITY_NAME="Git.GitforWindows.Snapshot"
	DISPLAY_NAME="Git for Windows (Snapshot)"
	;;
*rc*)
	IDENTITY_NAME="Git.GitforWindows.Prerelease"
	DISPLAY_NAME="Git for Windows (Prerelease)"
	;;
*)
	IDENTITY_NAME="Git.GitforWindows"
...
# MSIX versions are exactly major.minor.build.revision; keep the first four
# segments so snapshot versions with extra components remain valid.
MSIX_VERSION="$(echo "$MSIX_VERSION" | cut -d. -f1-4)"
...
ucrt64) pacman -Sy --noconfirm mingw-w64-ucrt-x86_64-librsvg;;
aarch64) pacman -Sy --noconfirm mingw-w64-clang-aarch64-librsvg;;
esac ||
die "Could not install librsvg"

# The MSIX tile logos are all square, so render them from the square icon-only
# mark rather than the wide git-for-windows.svg wordmark, which would be
# distorted when squeezed into a square.
SVG_SOURCE="$SCRIPT_PATH/git-icon.svg"
for spec in \
...
# A packaged Git installs under WindowsApps, so its prefix-relative system
# config never coincides with C:\Program Files\Git\etc\gitconfig, the canonical
# location that unpackaged Git for Windows (and the MinGit bundled by Visual
# Studio, GitHub Desktop, etc.) read. Include that file, when present, after the
# package's own defaults so an MSIX installed alongside a regular Git honors the
# same machine-wide settings; a missing include target is silently ignored.
git config -f "$SCRIPT_PATH/root/$etc_gitconfig" \
	--add include.path "C:/Program Files/Git/etc/gitconfig" ||
die "Could not add machine-wide gitconfig include"
...
sed -e "s/@@VERSION@@/$MSIX_VERSION/g" \
	-e "s/@@ARCHITECTURE@@/$ARTIFACT_SUFFIX/g" \
	-e "s/@@MINVERSION@@/$MIN_VERSION/g" \
	-e "s|@@IDENTITY_NAME@@|$IDENTITY_NAME|g" \
	-e "s|@@DISPLAY_NAME@@|$DISPLAY_NAME|g" \
	<"$MANIFESTIN" >"$MANIFESTOUT"
