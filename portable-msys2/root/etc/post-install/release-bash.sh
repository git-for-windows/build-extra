...
LIST="$(ARCH=$ARCH BITNESS=$BITNESS \
	...
	sh "$SCRIPT_PATH"/../make-file-list-bash.sh "$@" "pacman" |
...
LIST="var/lib/pacman/sync/* var/lib/pacman/local/*/* etc/pacman*/*"
...
(cat "$SCRIPT_PATH/../7-Zip/7zSD.sfx" &&
 echo ';!@Install@!UTF-8!' &&
 echo 'Title="Portable Bash for Windows '$BITNESS'-bit"' &&
 ...
 echo 'GUIMode="1"' &&
 echo 'InstallPath="%%S\\PortableBash"' &&
